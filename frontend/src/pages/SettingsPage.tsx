...
async function createCroppedImage(imageSrc: string, crop: Area) {
  ...
  return new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((blob) => (blob ? resolve(blob) : reject(new Error('Не удалось обрезать изображение'))), 'image/jpeg', 0.92);
  });
}
...
export function SettingsPage() {
  ...
  async function saveChanges(e: React.FormEvent) {
    ...
    try {
      const profileChanged =
        displayName !== savedDisplayName || nicknameColor !== savedNicknameColor || about !== savedAbout;
      const passwordChanged = Boolean(currentPassword || newPassword);

      if (profileChanged) {
        const updatedUser = await authApi.updateProfile(displayName, nicknameColor, about);
  ...
  function openCropper(e: React.ChangeEvent<HTMLInputElement>, target: CropTarget) {
  ...
  async function uploadCroppedImage() {
    ...
    try {
      if (cropTarget === 'avatar') {
        if (cropVideoFile) {
          const avatarUrl = await authApi.uploadAvatar(cropVideoFile, croppedAreaPixels);
      ...
      setError(`Не удалось загрузить ${cropTarget === 'avatar' ? 'аватар' : 'баннер'}`);
    } finally {
    ...
    <main className="settings-page">
      <div className="settings-card">
        ...
        <label className="settings-banner-upload" htmlFor="banner-upload">
          {user?.bannerUrl && <img src={`${import.meta.env.VITE_API_URL}${user.bannerUrl}`} alt="Баннер профиля" />}
          <span>{user?.bannerUrl ? 'Изменить баннер' : 'Добавить баннер'}</span>
        </label>
        <input id="banner-upload" className="settings-upload-input" type="file" accept="image/*" onChange={(e) => openCropper(e, 'banner')} />

        <form className="settings-form" onSubmit={saveChanges}>
          <div className="settings-field">
            <div className={`input-field${displayName ? ' input-field--filled' : ''}`}>
              <input
              ...
              <label htmlFor="display-name">
                {Array.from('Никнейм').map((character, index) => (
          ...
          <div className="settings-field">
            <label className="settings-label" htmlFor="about">О себе</label>
            <textarea className="input settings-about-input" id="about" value={about} maxLength={500} rows={4} onChange={(e) => setAbout(e.target.value)} placeholder="Расскажите немного о себе" />
          </div>

          <div className="settings-field">
            <span className="settings-label">Цвет ника</span>
          ...
          <div className="settings-field">
            <div className={`input-field${currentPassword ? ' input-field--filled' : ''}`}>
              <input
                className="input input--floating"
                id="current-password"
                type="password"
              ...
              <label htmlFor="current-password">
                {Array.from('Текущий пароль').map((character, index) => (
                  <span key={`${character}-${index}`} style={{ transitionDelay: `${index * 50}ms` }}>
                    {character}
                  </span>
                ))}
          ...
          <div className="settings-field">
            <div className={`input-field${newPassword ? ' input-field--filled' : ''}`}>
              <input
                className="input input--floating"
                id="new-password"
                type="password"
              ...
              <label htmlFor="new-password">
                {Array.from('Новый пароль').map((character, index) => (
                  <span key={`${character}-${index}`} style={{ transitionDelay: `${index * 50}ms` }}>
                    {character}
                  </span>
                ))}
        ...
        {message && (
          <p className="settings-message settings-message--success">{message}</p>
        )}
      ...
      {(cropImage || cropVideo) && cropTarget && createPortal(
        <div className="settings-crop-backdrop" role="dialog" aria-modal="true" aria-labelledby="crop-title">
          <div className="settings-crop-dialog">
            <div className="settings-crop-header">
              <h2 id="crop-title">Обрезать {cropTarget === 'avatar' ? 'аватар' : 'баннер'}</h2>
              <button className="btn btn-secondary" type="button" onClick={closeCropper} disabled={isUploading}>Отмена</button>
            </div>
            <div className={`settings-crop-area settings-crop-area--${cropTarget}`}>
              <Cropper
                ...
                showGrid
                mediaProps={{ muted: true, playsInline: true, autoPlay: true }}
            ...
            <div className="settings-crop-controls">
              <label>Масштаб <input type="range" min="1" max="3" step="0.1" value={zoom} onChange={(e) => setZoom(Number(e.target.value))} /></label>
            </div>
            <button className="btn btn-primary" type="button" onClick={uploadCroppedImage} disabled={isUploading || !croppedAreaPixels}>
