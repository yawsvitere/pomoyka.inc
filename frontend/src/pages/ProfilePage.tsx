...
function ProfileFileRow({ file }: { file: filesApi.UserFile }) {
  ...
  async function openFile() {
    ...
    setIsLoading(true);
    try {
      const loadedUrl = await filesApi.getAuthenticatedFileUrl(file.downloadUrl);
      setUrl(loadedUrl);
      if (newWindow) {
  ...
  return (
    <div className="fm-file-row profile-file-row">
      <span className="fm-file-name">
        <span className="fm-file-avatar">{fileInitials(file.fileName)}</span>
        <span className="fm-file-meta">
          ...
          <small>{file.contentType || file.fileName.split(".").pop()?.toUpperCase()}</small>
        </span>
      </span>
      <span className="fm-cell">{formatFileSize(file.sizeBytes)}</span>
      <span className="fm-cell">{new Date(file.uploadedAt).toLocaleDateString("ru-RU")}</span>
...
export function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (isCacheFresh) return () => { active = false; };
        ...
        <>
          <header className="profile-header">
            <div
              ...
              style={profile.bannerUrl ? { backgroundImage: `url(${getBrowserFileUrl(profile.bannerUrl)})` } : undefined}
            />
            ...
            {profile.avatarUrl ? (
              ...
              <div className="profile-details">
                <div className="profile-status-line">
                  <span className={`profile-status ${profile.isOnline ? "online" : "offline"}`}>
                    {profile.isOnline ? "Онлайн" : "Не в сети"}
                  </span>
                  {!profile.isOnline && profile.lastSeenAt && (
                    <span className="profile-last-seen">
                      Был в сети {new Date(profile.lastSeenAt).toLocaleString("ru-RU", { dateStyle: "short", timeStyle: "short" })}
                    </span>
                  )}
                </div>
                <p className="profile-email">{profile.email}</p>
                <h1 style={{ color: profile.nicknameColor ?? '#4f46e5' }}>{profile.displayName}</h1>
              ...
              <nav className="fm-breadcrumb profile-files-breadcrumb">
                ...
                {currentFolder && (
                  <>
                    ...
                    <span className="fm-breadcrumb-current">{currentFolder.name}</span>
                  </>
                )}
              </nav>
              ...
              <div className="fm-list-scroll">
                {visibleFolders.map((folder) => (
                  ...
                  >
                    ...
                    <span />
                    <span className="fm-cell">{new Date(folder.createdAt).toLocaleDateString("ru-RU")}</span>
                    <span className="fm-badge">{folder.accessLevel === 2 ? "Публичный" : "Братва"}</span>
                  </button>
                ))}
                {visibleFiles.map((file) => <ProfileFileRow key={file.id} file={file} />)}
                {visibleFolders.length === 0 && visibleFiles.length === 0 && <div className="fm-empty">Открытых файлов пока нет</div>}
              </div>
            </div>
          </section>
              ...
              <PostCard
                ...
                showAuthor={
                  ...
                  (posts[index - 1].author.id !== post.author.id &&
                    posts[index - 1].author.displayName !== post.author.displayName)
                }
                authorLinks
                showArticleLink
              />
            ))}
            {posts.length === 0 && <p className="profile-empty">Постишек пока нет.</p>}
          </section>
        </>
      ) : (
